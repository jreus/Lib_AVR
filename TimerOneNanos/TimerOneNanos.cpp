 ...
 */
#ifndef TIMERONENANOS_cpp
#define TIMERONENANOS_cpp

#include "TimerOneNanos.h"

TimerOneNanos Timer1;              // preinstatiate
...
void TimerOneNanos::initialize(long microseconds)
{
...
void TimerOneNanos::setPeriod(long microseconds)		// AR modified for atomic access
{
...
void TimerOneNanos::setPeriodNanos(long nanoseconds)		// AR modified for atomic access
{
  
  long cycles = (F_CPU / 2000000) * nanoseconds;                                     // the counter runs backwards after TOP, interrupt is at BOTTOM so divide microseconds by 2
  cycles /= 1000;
  if(cycles < RESOLUTION)              clockSelectBits = _BV(CS10);              // no prescale, full xtal
  else if((cycles >>= 3) < RESOLUTION) clockSelectBits = _BV(CS11);              // prescale by /8
  else if((cycles >>= 3) < RESOLUTION) clockSelectBits = _BV(CS11) | _BV(CS10);  // prescale by /64
...
void TimerOneNanos::setPwmDuty(char pin, int duty)
{
...
void TimerOneNanos::pwm(char pin, int duty, long microseconds)  // expects duty cycle to be 10 bit (1024)
{
...
void TimerOneNanos::disablePwm(char pin)
{
  if(pin == 1 || pin == 9)       TCCR1A &= ~_BV(COM1A1);   // clear the bit that enables pwm on PB1
  else if(pin == 2 || pin == 10) TCCR1A &= ~_BV(COM1B1);   // clear the bit that enables pwm on PB2
}

void TimerOneNanos::attachInterrupt(void (*isr)(), long microseconds)
{
  if(microseconds > 0) setPeriod(microseconds);
...
void TimerOneNanos::detachInterrupt()
{
  TIMSK1 &= ~_BV(TOIE1);                                   // clears the timer overflow interrupt enable bit 
															// timer continues to count without calling the isr
}

void TimerOneNanos::resume()				// AR suggested
{ 
  TCCR1B |= clockSelectBits;
}

void TimerOneNanos::restart()		// Depricated - Public interface to start at zero - Lex 10/9/2011
{
	start();				
}

void TimerOneNanos::start()	// AR addition, renamed by Lex to reflect it's actual role
{
  unsigned int tcnt1;
...
void TimerOneNanos::stop()
{
  TCCR1B &= ~(_BV(CS10) | _BV(CS11) | _BV(CS12));          // clears all clock selects bits
}

unsigned long TimerOneNanos::read()		//returns the value of the timer in microseconds
